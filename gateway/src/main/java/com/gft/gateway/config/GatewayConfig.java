...
public class GatewayConfig {
	...
	public RouteLocator routes(RouteLocatorBuilder builder) {
		return builder.routes().route("auth", r -> r.path("/auth/**").filters(f -> f.filter(filter)).uri("lb://auth"))
				...
				.route("diet-service", r -> r.path("/translate/**").filters(f -> f.filter(filter)).uri("lb://DIET-SERVICE"))
				.route("nutritionist-service", r -> r.path("/nutritionist/**").filters(f -> f.filter(filter)).uri("lb://NUTRITIONIST-SERVICE"))
				...
				.route("diets-groups-service", r -> r.path("/diets-groups/**").filters(f -> f.filter(filter)).uri("lb://DIETS-GROUPS-SERVICE")).build();
	}
}
