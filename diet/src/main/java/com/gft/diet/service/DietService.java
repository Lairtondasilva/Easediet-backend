...
public class DietService {
    ...
    public ResponseEntity<DietModel> registerDiet(DietModel diet){
        if (checkIfDietAlreadyExists(diet.getName())){
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST,"Diet name already used",null);
        }
        
    //Envio da List<FoodModel> para tradução para o inglês
        //Capturar a diet
        //Enviar os 9 campos para tradução
        //Pegar os campos traduzidos e armazenar nos nomes List<FoodModel>
        //Enviar os nomes da List<FoodModel> na query da API de calorias
        //
